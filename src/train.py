...
def train_and_get_best_model(misr_model, regis_model, optimizer, dataloaders, baseline_cpsnrs, config):
    """
    Trains MISR-GRU and ShiftNet for Multi-Image Super Resolution (MISR), and saves best model.
    Args:
        misr_model: torch.model, MISR-GRU
    ...
    subfolder_pattern = 'batch_{}_views_{}_min_{}_beta_{}'.format(
    ...
    for epoch in tqdm(range(1, num_epochs + 1)):
        ...
        for lrs, alphas, hrs, hr_maps, names in dataloaders['val']:
            ...
            srs = misr_model(lrs, alphas)[:, 0]  # fuse multi frames (B, 1, 3*W, 3*H)
        ...
        if best_score > val_score:
            torch.save(misr_model.state_dict(),
                       os.path.join(checkpoint_dir, 'MISR-GRU.pth'))
            torch.save(regis_model.state_dict(),
                       os.path.join(checkpoint_dir, 'ShiftNet.pth'))
...
def main(config):
    ...
    if os.path.exists(os.path.join(data_directory, "norm.csv")):
        baseline_cpsnrs = read_baseline_CPSNR(os.path.join(data_directory, "norm.csv"))

    train_set_directories = get_imageset_directories(os.path.join(data_directory, "train"))
